...
2. 英文原文应立即随人声逐句出现（先铺英文占位），中文译文翻到哪补到哪 ——
   不该出现「干等一阵后一大片字幕唰地全刷出来」
3. 播放器右上角出现「翻译中 n/m」徽标；再次打开同一视频秒出（缓存）
...
**想翻成中文以外的语言 / 改翻译风格？** 设置页「翻译」→「翻译提示词」就是发给
模型的系统提示词，直接改这段即可（比如把里面的语言换成日语）。`{lang}` 会替换成
上面选的目标语言、`{domain}` 替换成领域提示；输出格式、断句、难词等硬规则由程序
自动追加，不用你写。留空则用内置默认模板，点「恢复默认」可随时还原。改完当前视频
会用新提示词重新翻译（缓存按提示词内容区分，不会串味）。
